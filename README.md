...
*FortyGuard Hackathon '26 submission. Built on top of the [Temperature API Quickstart](#temperature-api-quickstart-base-template) template — [jump to those docs ↓](#temperature-api-quickstart-base-template).*

**Live demo:** https://coolcost-engine.streamlit.app/ · **Run locally:** `streamlit run app.py` (see [Getting started](#getting-started) below)
...
# Temperature API Quickstart (base template)

*This is the underlying template the project above was built from — kept
here for reference and for anyone using [`fortyguard/`](fortyguard/) or the
notebooks directly. Not required reading for the submission above.*
...
> **Starting a hackathon project?** Click **Use this template → Create a new repository** (green button, top of this page) to spin up your own copy to work in, then follow [Getting started (quickstart template)](#getting-started-quickstart-template) below.
...
## Getting started (quickstart template)
...
├── app.py                    # Data-Centre Siting & Cooling-Cost Engine (Streamlit)
├── dc_siting/                 # cost model + FortyGuard integration for the app above
├── docs/
...
│   ├── dc_siting_cache/       # pre-warmed cached responses for the 6 named DATS sites
